  ...
  swapCfg = {config, options, ...}: {
    ...
    options = {
      ...
      discardPolicy = mkOption {
        ...
        description = ''
        Specify the discard policy for the swap device. Can be one of "ones", "pages" or "both".
        If "once", then the whole swap space is discarded at swapon invocation. If "pages",
        asynchronous discard on freed pages is performed, before returning to the available pages
        pool. With "both", both policies are activated.
